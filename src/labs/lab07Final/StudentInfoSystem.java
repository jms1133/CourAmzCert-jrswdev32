...
public class StudentInfoSystem {
    ...
    public List<Subject> sortSubjectsByName() {
        List<Subject> subjectsByName = new ArrayList<>(db.subjects); //better way of cloning
    ...
    public List<Student> sortStudentsByName(){
        ...
        for (int unsortIdx = 1; unsortIdx < n; unsortIdx++){
            Student unsortedSt = students.get(unsortIdx);

            int sortedIdx = unsortIdx - 1;
            while ( sortedIdx >= 0 &&
                    students.get(sortedIdx).getName().
                    compareToIgnoreCase(unsortedSt.getName()) > 0) {

                students.set(sortedIdx + 1, students.get(sortedIdx));
                sortedIdx--;
            }
            students.set(sortedIdx + 1, unsortedSt);
        }
    ...
    public Student findStudent(Integer studentNumber) {
        ...
        while (l <= r) {
            int m = l + (r - l) / 2;
            int compare = students.get(m).getStudentNumber().compareTo(studentNumber);
            if (compare == 0) return students.get(m);
            if (compare < 0) {l = m + 1;}
            else {r = m - 1;}
        }
        return null;
